    ...
    with torch.no_grad():
        ...
        for index in range(len(dataset)):
            ...
            if torch.cuda.is_available():
                detections = retinanet(data["image"].cuda().float().unsqueeze(dim=0))
            else:
                detections = retinanet(data["image"].float().unsqueeze(dim=0))
...
def _get_annotations(generator):
    ...
    for i in range(len(generator)):
        ...
        annotations = generator[i]["target"]

        # copy detections to all_annotations
        for label in range(generator.num_classes()):
            all_annotations[i][label] = annotations["boxes"][annotations["labels"] == label]

...
def _resize_image_and_masks_onnx(image, self_min_size, self_max_size, target):
    ...
    image = torch.nn.functional.interpolate(
        image[None], scale_factor=scale_factor, mode="bilinear", recompute_scale_factor=True, align_corners=False,
    )[0]
...
def resize_keypoints(keypoints, original_size, new_size):
    ...
    ratios = [
        ...
        / torch.tensor(s_orig, dtype=torch.float32, device=keypoints.device)  # noqa: W503
...
def resize_boxes(boxes, original_size, new_size):
    # type: (Tensor, List[int], List[int]) -> Tensor
    ratios = [
        torch.tensor(s, dtype=torch.float32, device=boxes.device) / torch.tensor(s_orig, dtype=torch.float32, device=boxes.device)
        for s, s_orig in zip(new_size, original_size)
    ]

...
def _onnx_heatmaps_to_keypoints(
    ...
    xy_preds_i = torch.stack(
        [xy_preds_i_0.to(dtype=torch.float32), xy_preds_i_1.to(dtype=torch.float32), xy_preds_i_2.to(dtype=torch.float32),], 0,
    )
...
def heatmaps_to_keypoints(maps, rois):
    ...
    for i in range(len(rois)):
        ...
        roi_map = F.interpolate(maps[i][:, None], size=(roi_map_height, roi_map_width), mode="bicubic", align_corners=False,)[
            :, 0
        ]
